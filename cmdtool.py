...
parser.add_argument('-m','--use-mirror',help='Download the versions.json file from the specified URL',nargs=1,type='str')
...
group_type.add_argument('-R','--Remove',help='Clean up compiled build files (will be deleted along with compiled binaries!)',nargs=2,metavar=['Type of Server','Target Server version'],type='str')
group_type.add_argument('--RemoveALL',help='THIS COMMAND WILL REMOVE ALL FILES IN THE DIRECTORY EXCEPT THE PROGRAM BODY',nargs=2,metavar=['Type of Server','Target Server version'],type='str')
parser.parse_args()