...
# 获取服务端地址的函数
def get_server_address(url, data_key, is_download=False, download_path='', file_name='', ip_adress=''):
    ...
    try:
        ...
        for pane in download_panes:
            ...
            try:
                ...
                print('版本'+version+'地址'+_download_link)
    ...
    for version in versions:
        ...
        if is_download==True:
            ...
            if os.path.exists(download_path+'/'+filename):
                ...
                # 检查是否存在旧的构建文件
                pattern = re.compile(f"{project_id}-{re.escape(version)}-(.*).jar")
                ...
                if len(old_builds) > 1:
                    ...
                    for old_build in old_builds_info:
                        ...
                        if int(old_build) != int(latest_build):
                            # 删除旧构建文件
                            os.remove(os.path.join(download_path, f"{project_id}-{version}-{old_build}.jar"))
                            print(f"已删除旧构建文件 {project_id}-{version}-{old_build}.jar")
            _data[version] = ip_adress+'/'+filename
            # 更新下载链接
            #_data.clear()
            _data.update(_data)
    ...
    for version in versions:
        ...
        if is_download==True:
            ...
            if os.path.exists(download_path+'/'+filename):
                ...
                # 定义文件名和保存路径
                _file_name=f'/purpur-{version}-{latest_build}.jar'
                _download_path = download_path+_file_name
                # 下载构建文件
                urllib.request.urlretrieve(download_url, _download_path)
                print(f"已下载 {filename} !")
            ...
            # 检查是否存在旧的构建文件
            pattern = re.compile(f"purpur-{re.escape(version)}-(.*).jar")
            old_builds = glob.glob(os.path.join(download_path, "*.jar"))
            ...
            if len(old_builds) > 1:
                ...
                for old_build in old_builds_info:
                    ...
                    if int(old_build) != int(latest_build):
                        # 删除旧构建文件
                        os.remove(os.path.join(download_path, f"purpur-{version}-{old_build}.jar"))
                        print(f"已删除旧构建文件 purpur-{version}-{old_build}.jar")
            '''
            # 检查是否存在旧的构建文件
            old_builds = glob.glob(download_path+"/"+f"purpur-{version}-*.jar")
            if len(old_builds) > 1:
                # 获取旧构建的版本和编号信息
                ...
                for old_build in old_builds_info:
                    ...
                    if int(old_build) != int(latest_build):
                        # 删除旧构建文件
                        os.remove(download_path+"/"+f"purpur-{version}-{old_build}.jar")
                        print(f"已删除旧构建文件 purpur-{version}-{old_build}.jar")
            '''
            _data[version] = ip_adress+'/'+filename
            # 更新下载链接
...
def get_server_spigot():
    get_server_address('https://getbukkit.org/download/spigot', 'Spigot（插件服务端）',True,"html/files/servers/spigot","spigot","http://47.243.96.125/files/servers/spigot")
    #get_server_address('https://getbukkit.org/download/spigot', 'Spigot（插件服务端）')

def get_server_vanilla():
    get_server_address('https://getbukkit.org/download/vanilla', 'Vanilla（原版服务端）')

def get_server_bukkit():
    get_server_address('https://getbukkit.org/download/craftbukkit', 'CraftBukkit（插件服务端）')

def get_server_paper():
    download_latest_builds('paper', 'Paper（插件服务端）',True,"html/files/servers/paper","http://47.243.96.125/files/servers/paper")
    #download_latest_builds('paper', 'Paper（插件服务端）')

def get_server_purpur():
    download_latest_builds_purpur('Purpur（插件服务端）',True,"html/files/servers/purpur","http://47.243.96.125/files/servers/purpur")
    #download_latest_builds_purpur('Purpur（插件服务端）')

def get_server_folia():
    download_latest_builds('folia', 'Folia（多线程插件服务端）',True,"html/files/servers/folia","http://47.243.96.125/files/servers/folia")
    #download_latest_builds('folia', 'Folia（多线程插件服务端）')

def get_server_velocity():
    download_latest_builds('velocity', 'Velocity（代理服务端，单端勿用）',True,"html/files/servers/velocity","http://47.243.96.125/files/servers/velocity")
...
def get_server_mohist():
    download_latest_build_mohist('Mohist（模组插件二合一服务端）',True,"html/files/servers/mohist","http://47.243.96.125/files/servers/mohist")
...
# 定义定时执行的函数列表和时间间隔（单位：秒）
functions = [get_server_paper, get_server_purpur, get_server_spigot, get_server_bukkit, get_server_vanilla, get_server_folia, get_server_fabric, get_server_forge, get_server_mohist, get_server_velocity]
