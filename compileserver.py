...
COMPILE_ROOT = os.getcwd()+os.sep+'Compile'+os.sep
...
def clone_catserver():
    ...
    if not os.path.exists("BuildTools.jar"):
        ...
        log("已存在BuildTools.jar,将不会再次进行下载",2)

'''编译Paper&Folia'''
def compile_paper():
    for pp_ver in paper_versions:
        log(f"开始编译Paper,版本:{pp_ver}")
        sp.run(['./gradlew applyPatches','./gradlew createReobfBundlerJar'],cwd=f'{COMPILE_ROOT}Paper{os.sep}')
    log("开始编译Folia,latest")
    sp.run(['./gradlew applyPatches','./gradlew createReobfBundlerJar'],cwd=f'{COMPILE_ROOT}Paper{os.sep}')
...
def compile_spigot():
    for version in spigot_version_list:
        ...
        sp.run(['java','-jar','BuildTools.jar',f'--rev={target-version}',f'--output-dir="{COMPILE_ROOT}Spigot{os.sep}"'])
        if eval(version) >= 1.14:
            '''编译CraftBukkit(1.14及以上)'''
            sp.run(['java','-jar','BuildTools.jar',f'--rev={target-version}',f'--output-dir="{COMPILE_ROOT}Spigot{os.sep}"','--compile craftbukkit'])
        
'''编译Mohist'''
def compile_mohist():
    for mo_ver in mohist_versions:
        log(f"开始编译Mohist,版本:{mo_ver}")
        if mo_ver == '1.7.10':
            sp.run(['./gradlew setupCauldron launch4j'],cwd=f'Compile/Mohist-{mo_ver}/')
        elif mo_ver == '1.12.2':
            sp.run(['./gradlew setup installerJar'],cwd=f'Compile/Mohist-{mo_ver}/')
        else:
            sp.run(['./gradlew setup mohist.jar'],cwd=f'Compile/Mohist-{mo_ver}/')
        
'''编译Catserver'''
def compile_catserver():
    for ct_ver in catserver_versions:
        log(f"开始编译Catserver,版本:{mo_ver}")
        sp.run(['./gradlew setup','./gradlew genPatches','./gradlew build'],cwd=f'Compile/Catserver-{ct_ver}')
    
if __name__ == '__main__':
    log(f'COMPILE_ROOT:{COMPILE_ROOT}')
    log('正在检测是否已有服务端仓库')
    test_repo()
    log('编译Paper&Folia')
    compile_paper()
